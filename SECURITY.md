...
fbrain stores sensitive personal data — email metadata, professional profile, synthesized identity. This document explains what we protect against, what we don't, and what you should know.

### What fbrain protects against

- **Cloud breach**: Your data is never sent to any fbrain server. There is no fbrain server.
...
### What fbrain does NOT protect against
...
|---|---|---|
| imap.gmail.com | Your Gmail credentials, your IMAP queries | During `fbrain sync gmail` |
| generativelanguage.googleapis.com | Synthesized email summary (no raw content), LinkedIn PDF text | During `fbrain sync` |
| Nothing else | — | — |

fbrain makes no other network requests.
...
The manifest records fbrain-managed files with repo-relative paths and hashes
...
fbrain-managed hooks, runtime files, root config, workflow, and manifest; it
